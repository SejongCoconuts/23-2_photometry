...
dd1 = pd.DataFrame({'time':[tt], 'flux':[flux]})
df1 = dd1.apply(pd.Series.explode)
df1.to_csv("Mkn501_rs.txt",sep="\t", index = False) 
...
xx = tt
yy = flux
plt.title('Simulation(removed data) LC')
plt.xlabel('Julian Date')
plt.ylabel('Photon Flux [0.1-100 GeV](photons cm-2 s-1)')
plt.grid()
plt.plot(xx,yy)
#plt.show()
plt.savefig('Simulation(removed data) LC.png')
...
plt.title('Simulation(removed data) PSD')
plt.scatter(freq,PSD)
plt.xlabel('log_freq')
plt.ylabel('log_PSD')
#plt.show()
plt.savefig('removed_data_PSDs.png')
...
plt.grid()
plt.show()
#plt.savefig('removed_data_PLs.png')